...
- 当时唯一阻断点是 inference tensor 反向；该问题现已修复并有回归测试。
...
1. 在 L4 拉取最新代码后跑 100–500 optimizer step，确认修复后的第一条
   loss、W&B 曲线、step 吞吐和 Drive mirror；这是唯一缺少的真实线上长跑。
2. 用 `scripts/benchmark_l4_attention.py` 记录该 Colab runtime 的 FA2 对
   SDPA 实测；不同 PyTorch/driver 下结果会变化。
3. 增加固定 validation prompt/grid 的周期性异步采样；当前已有手动采样。
